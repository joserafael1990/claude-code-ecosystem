...
const title = 'Claude Code Ecosystem';
const subtitle = 'The Definitive Curated Extension for Claude Code';

const colorGradient = ['#6366F1', '#818CF8', '#A5B4FC', '#C7D2FE', '#E0E7FF', '#EEF2FF'];
...
function showBanner() {
  ...
  console.log('\n');
  console.log('       ' + chalk.hex('#818CF8')(subtitle));
  console.log('\n');
  console.log(chalk.hex('#6366F1')('════════════════════════════════════════════════════════════════\n'));

  console.log(
    chalk.hex('#4F46E5')('✨ Curated Agents • ⚡ Specialized Commands • 🔌 MCP Integrations ✨') +
    chalk.gray(`\n                             v${pkg.version}\n`)
  );
}
...
program
  ...
  .description('The definitive extension for Claude Code - Install curated agents, commands, and MCPs.')
  ...
  .option('--global', 'install components globally (available across all projects)')
  .option('--agent <agent>', 'install specific agent component (supports multiple flags or comma-separated)', (val, memo) => { memo.push(val); return memo; }, [])
  .option('--command <command>', 'install specific command component (supports multiple flags or comma-separated)', (val, memo) => { memo.push(val); return memo; }, [])
  .option('--mcp <mcp>', 'install specific MCP component (supports multiple flags or comma-separated)', (val, memo) => { memo.push(val); return memo; }, [])
  .option('--setting <setting>', 'install specific setting component (supports multiple flags or comma-separated)', (val, memo) => { memo.push(val); return memo; }, [])
  .option('--hook <hook>', 'install specific hook component (supports multiple flags or comma-separated)', (val, memo) => { memo.push(val); return memo; }, [])
  .option('--skill <skill>', 'install specific skill component (supports multiple flags or comma-separated)', (val, memo) => { memo.push(val); return memo; }, [])
