...
npx claude-code-ecosystem@latest --sandbox e2b \
...
npx claude-code-ecosystem@latest --sandbox e2b --prompt "Create a React todo app"
```

### Cloudflare Sandbox
```bash
# Execute via deployed Cloudflare Worker
npx claude-code-ecosystem@latest --sandbox cloudflare \
  --anthropic-api-key your_anthropic_key \
...
```bash
npx claude-code-ecosystem@latest --sandbox e2b --prompt "Create an HTML page with modern CSS animations"
```
...
The sandbox system integrates seamlessly with the existing Claude Code Ecosystem component architecture, allowing any combination of agents, commands, MCPs, settings, and hooks to be installed and used within the secure sandbox environment.
