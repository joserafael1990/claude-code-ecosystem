...
function displayCommandStats(analysis) {
  ...
  if (!analysis.exists) {
    ...
    console.log(chalk.blue('\n💡 Run the setup first: npx claude-code-ecosystem'));
...
/**
 * Prompts user to setup Claude Code Ecosystem when no commands are found
...
async function promptSetupWhenNoCommands(targetDir) {
  ...
  console.log(chalk.gray('No Claude Code commands found in this project. You can set up Claude Code Ecosystem to get started.'));

  try {
    const { setupNow } = await inquirer.prompt([{
      type: 'confirm',
      name: 'setupNow',
      message: 'Would you like to start the Claude Code Ecosystem setup now?',
      default: true,
      prefix: chalk.blue('🤖')
    }]);

    if (!setupNow) {
      console.log(chalk.yellow('⏭️  Setup skipped. Run "npx claude-code-ecosystem" anytime to set up your project.'));
      return false;
    }

    console.log(chalk.blue('\n🚀 Starting Claude Code Ecosystem setup...'));
    ...
    console.log(chalk.blue('💡 You can run setup manually with: npx claude-code-ecosystem'));
    return false;
  }
}
