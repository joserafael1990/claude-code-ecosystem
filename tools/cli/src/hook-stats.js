...
async function runHookStats(options) {
  ...
  if (!analysis) {
    ...
    console.log(chalk.gray('Would you like to set up Claude Code Ecosystem to add automation hooks?'));
    
    const { setupHooks } = await inquirer.prompt([{
      type: 'confirm',
      name: 'setupHooks',
      message: 'Set up automation hooks with Claude Code Ecosystem?',
      default: true
    }]);
