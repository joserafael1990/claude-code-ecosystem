...
const GLOBAL_AGENTS_DIR = path.join(os.homedir(), '.claude-code-ecosystem');
...
async function listGlobalAgents(options = {}) {
  ...
  try {
    ...
    if (allAgents.length === 0) {
      ...
      console.log(chalk.gray('💡 Create one with: npx claude-code-ecosystem@latest --create-agent <agent-name>'));
      return;
    }
    ...
    console.log(chalk.gray('  • List agents: npx claude-code-ecosystem@latest --list-agents'));
    console.log(chalk.gray('  • Remove agent: npx claude-code-ecosystem@latest --remove-agent <name>'));
...
async function addToPath() {
  ...
  for (const configFile of configFiles) {
    try {
      ...
      if (!content.includes(BIN_DIR)) {
        const newContent = content + `\n# Claude Code Ecosystem - Global Agents\n${pathExport}\n`;
...
async function findAgentUrl(agentName) {
  try {
    ...
    const rootUrl = `https://raw.githubusercontent.com/joserafael1990/claude-code-ecosystem/main/cli-tool/components/agents/${agentName}.md`;
    ...
    const categoriesResponse = await fetch('https://api.github.com/repos/joserafael1990/claude-code-ecosystem/contents/cli-tool/components/agents');
    ...
    for (const item of contents) {
      if (item.type === 'dir') {
        const categoryUrl = `https://raw.githubusercontent.com/joserafael1990/claude-code-ecosystem/main/cli-tool/components/agents/${item.name}/${agentName}.md`;
        try {
...
async function showAvailableAgents() {
  ...
  try {
    ...
    console.log(chalk.gray('  npx claude-code-ecosystem@latest --create-agent api-security-audit'));
    console.log(chalk.gray('  npx claude-code-ecosystem@latest --create-agent deep-research-team/academic-researcher'));
