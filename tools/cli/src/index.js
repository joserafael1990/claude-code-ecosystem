...
async function showMainMenu() {
  ...
  const initialChoice = await inquirer.prompt([{
    ...
    choices: [
      {
        name: '📊 Analytics Dashboard - Monitor your Claude Code Ecosystem usage and sessions',
        value: 'analytics',
      ...
      {
        name: '⚙️ Project Setup - Configure Claude Code Ecosystem for your project',
        value: 'setup',
        short: 'Project Setup'
      },
      {
        name: '🔍 Health Check - Verify your Claude Code Ecosystem setup and configuration',
        value: 'health',
        short: 'Health Check'
      }
    ],
  ...
  if (initialChoice.action === 'analytics') {
    console.log(chalk.blue('📊 Launching Claude Code Ecosystem Analytics Dashboard...'));
  ...
  if (initialChoice.action === 'chats') {
    console.log(chalk.blue('💬 Launching Claude Code Ecosystem Mobile Chats...'));
  ...
  if (initialChoice.action === 'agents') {
    console.log(chalk.blue('🤖 Launching Claude Code Ecosystem Agents Dashboard...'));
  ...
  console.log(chalk.blue('⚙️  Setting up Claude Code Ecosystem configuration...'));
...
async function createClaudeConfig(options = {}) {
  ...
  if (options.global) {
    ...
    console.log(chalk.yellow(`🌍 Global installation mode enabled. Target: ${targetDir}`));
  }
  ...
  if ((options.agent && options.agent.length > 0) ||
    ...
    (options.setting && options.setting.length > 0) ||
    (options.hook && options.hook.length > 0) ||
  ...
  if (options.cloneSession) {
    console.log(chalk.blue('📥 Cloning shared Claude Code Ecosystem session...'));
  ...
  if (!shouldRunSetup && !options.setupFromMenu && !options.yes && !options.language && !options.framework && !options.dryRun) {
    ...
    console.log(chalk.blue('🚀 Setting up Claude Code Ecosystem configuration...'));
  }
  ...
  if (config.agents && config.agents.length > 0) {
    console.log(chalk.blue('🤖 Installing Claude Code Ecosystem agents...'));
  ...
  console.log(chalk.green('✅ Claude Code Ecosystem configuration setup complete!'));
...
async function installIndividualAgent(agentName, targetDir, options) {
  ...
  try {
    ...
    // Try local first
    try {
      // Look in components/agents/ directory relative to this script
      const localBaseDir = path.join(__dirname, '../../../components/agents');

      // If agentName has no category, we need to search for it
      let localPath;
      if (agentName.includes('/')) {
        localPath = path.join(localBaseDir, `${agentName}.md`);
      } else {
        // Search through categories
        const categories = await fs.readdir(localBaseDir);
        for (const cat of categories) {
          const catPath = path.join(localBaseDir, cat);
          if ((await fs.stat(catPath)).isDirectory()) {
            const possiblePath = path.join(catPath, `${agentName}.md`);
      ...
      if (localPath && await fs.pathExists(localPath)) {
        console.log(chalk.gray(`📂 Found local component: ${path.relative(process.cwd(), localPath)}`));
        agentContent = await fs.readFile(localPath, 'utf8');
        isLocal = true;
      }
    } catch (err) {
      // Silence local lookup errors
    }
    ...
    if (!isLocal) {
      ...
      agentContent = await response.text();
    }
    ...
    if (!options.silent) {
      ...
      if (isLocal) {
        console.log(chalk.cyan(`📂 Source: Local component`));
      } else {
    ...
    trackingService.trackDownload('agent', agentName, {
      ...
      source: isLocal ? 'local_filesystem' : 'github_main'
    });
...
async function installIndividualCommand(commandName, targetDir, options) {
  ...
  try {
    ...
    // Try local first
    try {
      const localBaseDir = path.join(__dirname, '../../../components/commands');
      let localPath;
      if (commandName.includes('/')) {
        localPath = path.join(localBaseDir, `${commandName}.md`);
      } else {
        // Search through categories
        const categories = await fs.readdir(localBaseDir);
        for (const cat of categories) {
          const catPath = path.join(localBaseDir, cat);
          if ((await fs.stat(catPath)).isDirectory()) {
            const possiblePath = path.join(catPath, `${commandName}.md`);
            if (await fs.pathExists(possiblePath)) {
              localPath = possiblePath;
              break;
            }
      ...
      if (localPath && await fs.pathExists(localPath)) {
        console.log(chalk.gray(`📂 Found local component: ${path.relative(process.cwd(), localPath)}`));
        commandContent = await fs.readFile(localPath, 'utf8');
        isLocal = true;
      }
    } catch (err) {
      // Silence local lookup errors
    ...
    if (!isLocal) {
      ...
      commandContent = await response.text();
    }
...
async function installIndividualMCP(mcpName, targetDir, options) {
  ...
  try {
    ...
    // Try local first
    try {
      const localBaseDir = path.join(__dirname, '../../../components/mcps');
      let localPath;
      if (mcpName.includes('/')) {
        localPath = path.join(localBaseDir, `${mcpName}.json`);
      } else {
        // Search through categories
        const categories = await fs.readdir(localBaseDir);
        for (const cat of categories) {
          const catPath = path.join(localBaseDir, cat);
          if ((await fs.stat(catPath)).isDirectory()) {
            const possiblePath = path.join(catPath, `${mcpName}.json`);
            if (await fs.pathExists(possiblePath)) {
              localPath = possiblePath;
              break;
            }
      ...
      if (localPath && await fs.pathExists(localPath)) {
        console.log(chalk.gray(`📂 Found local component: ${path.relative(process.cwd(), localPath)}`));
        mcpConfigText = await fs.readFile(localPath, 'utf8');
        isLocal = true;
      }
    } catch (err) {
      // Silence local lookup errors
    ...
    if (!isLocal) {
      ...
      mcpConfigText = await response.text();
    }
...
async function installIndividualSetting(settingName, targetDir, options) {
  ...
  try {
    let settingConfigText;
    let isLocal = false;

    // Try local first
    try {
      const localBaseDir = path.join(__dirname, '../../../components/settings');
      let localPath;
      if (settingName.includes('/')) {
        localPath = path.join(localBaseDir, `${settingName}.json`);
      } else {
        // Search through categories
        const categories = await fs.readdir(localBaseDir);
        for (const cat of categories) {
          const catPath = path.join(localBaseDir, cat);
          if ((await fs.stat(catPath)).isDirectory()) {
            const possiblePath = path.join(catPath, `${settingName}.json`);
            if (await fs.pathExists(possiblePath)) {
              localPath = possiblePath;
              break;
            }
      ...
      if (localPath && await fs.pathExists(localPath)) {
        console.log(chalk.gray(`📂 Found local component: ${path.relative(process.cwd(), localPath)}`));
        settingConfigText = await fs.readFile(localPath, 'utf8');
        isLocal = true;
      }
    } catch (err) {
      // Silence local lookup errors
    ...
    if (!isLocal) {
      ...
      settingConfigText = await response.text();
    }
...
async function installIndividualHook(hookName, targetDir, options) {
  ...
  try {
    ...
    // Try local first
    try {
      const localBaseDir = path.join(__dirname, '../../../components/hooks');
      let localPath;
      if (hookName.includes('/')) {
        localPath = path.join(localBaseDir, `${hookName}.json`);
      } else {
        // Search through categories
        const categories = await fs.readdir(localBaseDir);
        for (const cat of categories) {
          const catPath = path.join(localBaseDir, cat);
          if ((await fs.stat(catPath)).isDirectory()) {
            const possiblePath = path.join(catPath, `${hookName}.json`);
            if (await fs.pathExists(possiblePath)) {
              localPath = possiblePath;
              break;
            }
      ...
      if (localPath && await fs.pathExists(localPath)) {
        console.log(chalk.gray(`📂 Found local component: ${path.relative(process.cwd(), localPath)}`));
        hookConfigText = await fs.readFile(localPath, 'utf8');
        isLocal = true;
      }
    } catch (err) {
      // Silence local lookup errors
    ...
    if (!isLocal) {
      ...
      hookConfigText = await response.text();
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        ...
        if (responseText.includes('rate limit')) {
          ...
          console.log(chalk.yellow('💡 Install locally with: npm install -g claude-code-ecosystem'));
      ...
      try {
        const response = await fetch(apiUrl, {
          headers: {
            ...
            'User-Agent': 'claude-code-ecosystem'
          }
        });
