...
class AgentAnalytics {
  constructor(container, services) {
    ...
    if (this.dataService && this.dataService.addEventListener) {
      this.dataService.addEventListener((type, data) => {
        if (type === 'data_refresh' || type === 'conversations') {
          this.handleDataRefresh(data, type);
        }
      });
    }
  }
      ...
      <div class="agent-analytics">
        ...
        <div class="analytics-header">
          <div class="header-content">
            ...
            <p class="analytics-subtitle">Specialized Claude Code Ecosystem agent usage patterns and workflow insights</p>
          </div>
  ...
  async loadAgentData() {
    try {
      ...
      if (this.agentData) {
        // Handle Catalog Mode (new backend)
        if (this.agentData.agents && Array.isArray(this.agentData.agents)) {
          this.renderCatalog(this.agentData.agents);
        }
        // Handle Analytics Mode (legacy backend)
        else if (this.agentData.summary) {
          this.updateMetrics();
          this.renderCharts();
  ...
  /**
   * Render Agent Catalog (Discovery Mode)
  ...
  renderCatalog(agents) {
      ...
      <div class="agent-analytics">
        <div class="analytics-header">
          <div class="header-content">
            ...
            <p class="analytics-subtitle">Discover and list available Claude Code Ecosystem agents (${agents.length} available)</p>
          </div>
          <button class="refresh-btn" id="refresh-analytics">
        ...
        <div class="agent-stats-grid" style="grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));">
            ${agents.map(agent => `
              <div class="agent-stat-card">
                <div class="agent-stat-header">
                  <div class="agent-stat-icon" style="background:${agent.color}20; color:${agent.color}">🤖</div>
                  <div class="agent-stat-info">
                    <h4 style="color:${agent.color}">${agent.name}</h4>
                    <p>${agent.description || 'No description'}</p>
                  </div>
                  <div class="agent-stat-badge">
                    ${agent.category || agent.level || 'System'}
                  </div>
                </div>
                <div class="agent-stat-metrics">
                   <div class="stat-metric">
                     <span class="metric-label">Type:</span>
                     <span class="metric-value">${agent.category || 'Standard'}</span>
                   </div>
                   <div class="stat-metric">
                     <span class="metric-label">Tools:</span>
                     <span class="metric-value">${agent.tools ? agent.tools.length : 0}</span>
                   </div>
                </div>
              </div>
            `).join('')}
    ...
    // Re-attach listeners for the new refresh button
    this.setupEventListeners();
  }
    ...
    this.container.innerHTML = `
      <div class="no-agent-data">
        <div class="no-data-content">
          ...
          <p>No specialized Claude Code Ecosystem agents have been used yet.</p>
