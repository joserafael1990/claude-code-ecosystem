...
# Test script for claude-code-ecosystem CLI
set -e
...
claude-code-ecosystem --help > /dev/null && echo "✅ Help command works" || echo "❌ Help command failed"

# Test 2: Version command
echo "🔸 Test 2: Version command"
claude-code-ecosystem --version > /dev/null && echo "✅ Version command works" || echo "❌ Version command failed"

# Test 3: Dry run with JavaScript/TypeScript + React
echo "🔸 Test 3: Dry run - JavaScript/TypeScript + React"
claude-code-ecosystem --language javascript-typescript --framework react --dry-run --yes > /dev/null && echo "✅ JS/TS + React dry run works" || echo "❌ JS/TS + React dry run failed"

# Test 4: Dry run with Common language
echo "🔸 Test 4: Dry run - Common language"
claude-code-ecosystem --language common --dry-run --yes > /dev/null && echo "✅ Common language dry run works" || echo "❌ Common language dry run failed"

# Test 5: Actual installation to test directory
echo "🔸 Test 5: Actual installation"
mkdir react-test-project
cd react-test-project
claude-code-ecosystem --language javascript-typescript --framework react --yes > /dev/null
...
timeout 5s claude-code-ecosystem --dry-run || echo "✅ Interactive mode starts correctly"
